...
class Write_excel(object):
    ...
    def wsheet(self,num=0):
        self.wb._active_sheet_index=num
        self.ws = self.wb.active

    def write(self, rx,cx, value):
        # eg: coord:A1
        self. ws.cell(row=rx, column=cx).value=value
        self.wb.save(self.filename)
    def pvalue(self,rx,cx):
        value=self. ws.cell(row=rx, column=cx).value
        return value
